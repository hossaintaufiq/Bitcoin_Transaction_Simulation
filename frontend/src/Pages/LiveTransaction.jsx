...
const LiveTransaction = ({ transactionHistory }) => {
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Transactions</h1>
            ...
            {transactionHistory.length === 0 ? (
              ...
              transactionHistory.map((tx, index) => (
                <tr key={index} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 text-sm text-gray-900">TX{100000 + index}</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{tx.amount.toFixed(6)}</td>
                  <td className="px-6 py-4 text-sm">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        tx.type === "buy"
